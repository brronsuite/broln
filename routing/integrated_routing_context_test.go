...
func (m *mockBandwidthHints) availableChanBandwidth(channelID uint64,
	_ lnwire.MilliBronees) (lnwire.MilliBronees, bool) {
...
func (c *integratedRoutingContext) testPayment(maxParts uint32,
	...
	getBandwidthHints := func(_ routingGraph) (bandwidthHints, error) {
		...
		bandwidthHints := map[uint64]lnwire.MilliBronees{}
	...
	payment := LightningPayment{
		...
		FeeLimit:       lnwire.MaxMilliBronees,
	...
	session.minShardAmt = lnwire.NewMSatFromBroneess(5000)
	...
	for {
		...
		route, err := session.RequestRoute(
			amtRemaining, lnwire.MaxMilliBronees, inFlightHtlcs, 0,
		)
		if err != nil {
			return attempts, err
		}
