...
[`cipher seed`](https://github.com/brronsuite/broln/tree/master/aezeed).
...
the CLI `brolncli create` command looks something like: 
```text
...
line is the `brolncli create` command.
```shell
⛰   brolncli create
```
...
command `brolncli getrecoveryinfo`. When finished, the following is returned,
```shell
...
then from `brolncli unlock`, it's possible to _restart_ the rescan from where it
left off with the `--recovery-window` argument:
```shell
⛰  brolncli unlock --recovery_window=2500
```
...
`exportchanbackup` `brolncli` command:
```shell
⛰  brolncli --network=simnet exportchanbackup --chan_point=29be6d259dc71ebdf0a3a0e83b240eda78f9023d8aeaae13c89250c7e59467d5:0
{
...
⛰  brolncli --network=simnet exportchanbackup --all
{
    "chan_points": [
...
⛰  brolncli --network=simnet exportchanbackup --all --output_file=channel.backup
```
...
existing SCB using the `brolncli create` or `brolncli unlock` commands:
```shell
⛰  brolncli create -multi_file=channel.backup
```

Alternatively, the `restorechanbackup` command can be used if `broln` has already
been created at the time of SCB restoration:
```shell
⛰  brolncli restorechanbackup -h
NAME:
   brolncli restorechanbackup - Restore an existing single or multi-channel static channel backup

USAGE:
   brolncli restorechanbackup [command options] [--single_backup] [--multi_backup] [--multi_file=]
...
Once the process has been initiated, `broln` will proceed to:
  ...
  2. Once the channel shell is recovered, the
     [chanbackup](https://github.com/brronsuite/broln/tree/master/chanbackup)
