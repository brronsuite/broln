...
sources](https://github.com/brronsuite/broln/blob/master/lnrpc/lightning.proto).
...
function paymentSender(destination, amount) {
  return function(callback) {
    console.log("Sending " + amount + " broneess");
...
let lnrpcDescriptor = grpc.loadPackageDefinition(packageDefinition);
let lnrpc = lnrpcDescriptor.lnrpc;
let client = new lnrpc.Lightning('some.address:10019', credentials);
