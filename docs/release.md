...
```shell
⛰  git clone https://github.com/brronsuite/broln.git
⛰  cd broln
...
6. Obtain a copy of `broln`'s source code with `git clone
   https://github.com/brronsuite/broln` and checkout the source code of the
...
8. Extract the archive found in the `broln-<TAG>` directory created by the
   ...
   and brolncli) with `shasum -a 256 <filename>`. These should match __exactly__
   as the ones noted above.

## Verifying Docker Images

To verify the `broln` and `brolncli` binaries inside the
