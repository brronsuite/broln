...
import (
	...
	"github.com/brronsuite/broln/blockcache"
	...
	"github.com/brronsuite/broln/watchtower/wtdb"
	...
	"github.com/brronsuite/bronwallet/walletdb"
	"github.com/brronsuite/neutrino"
	"github.com/brronsuite/neutrino/headerfs"
	...
	*bronwallet.Config, func(), error) {
	...
	if !d.cfg.NoMacaroons {
		...
		if !walletInitParams.StatelessInit &&
			...
			// Create macaroon files for brolncli to use if they don't
			// exist.
			err = genMacaroons(
...
func waitForWalletPassword(cfg *Config,
	...
	*walletunlocker.WalletUnlockParams, error) {
	...
	ltndLog.Infof("Waiting for wallet encryption password. Use `brolncli " +
		"create` to create a wallet, `brolncli unlock` to unlock an " +
		"existing wallet, or `brolncli changepassword` to change the " +
