...
var profileSubCommand = cli.Command{
	Name:     "profile",
	Category: "Profiles",
	Usage:    "Create and manage brolncli profiles.",
	Description: `
	Profiles for brolncli are an easy and comfortable way to manage multiple
	...
	short version '-p=myprofile') with any brolncli command.

	A default profile can also be defined, brolncli will then always use the
	...
	brolncli! To disable the use of the default profile for a single command,
	set '--profile= '.

	The profiles are stored in a file called profiles.json in the user's
	home directory, for example:
		C:\Users\<username>\AppData\Local\Brolncli\profiles.json on Windows
		~/.brolncli/profiles.json on Linux
		~/Library/Application Support/Brolncli/profiles.json on MacOS
	`,
...
var profileListCommand = cli.Command{
	Name:   "list",
	Usage:  "Lists all brolncli profiles",
	Action: profileList,
}
...
var profileAddCommand = cli.Command{
	...
	(see 'brolncli --help') passed into this command are stored in that named
	profile.
	`,
	Flags: []cli.Flag{
		cli.StringFlag{
...
var profileUnsetDefaultCommand = cli.Command{
	...
	Description: `
	Disables the use of a default profile and restores brolncli to its original
	behavior.
	`,
