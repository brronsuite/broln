// Copyright (c) 2013-2017 The brronsuite developers
...
import (
	...
	"github.com/brronsuite/broln/walletunlocker"
...
const (
	...
	// separate group for each user. E.g. a new user broln gets assigned group
	// broln which nothing else belongs to. A system that does not do this is
...
type ListenerWithSignal struct {
	...
	// that started broln. The channel should be buffered to avoid broln being
	// blocked on sending to the channel.
	MacChan chan []byte
}

// ListenerCfg is a wrapper around custom listeners that can be passed to broln
...
// Main is the true entry point for broln. It accepts a fully populated and
...
func Main(cfg *Config, lisCfg ListenerCfg, implCfg *ImplementationCfg,
	...
	// we direct broln to connect to its loopback address rather than a
	...
	// server and the regular broln server.
...
func getTLSConfig(cfg *Config) ([]grpc.ServerOption, []grpc.DialOption,
	...
	if !fileExists(cfg.TLSCertPath) && !fileExists(cfg.TLSKeyPath) {
		...
		err := cert.GenCertPair(
			"broln autogenerated cert", cfg.TLSCertPath,
...
// This function is taken from https://github.com/brronsuite/brond
...
func startRestProxy(cfg *Config, rpcServer *rpcServer, restDialOpts []grpc.DialOption,
	...
	restHandler := lnrpc.NewWebSocketProxy(
		...
		lnrpc.brolnClientStreamingURIs,
	)
