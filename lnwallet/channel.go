...
import (
	...
	"github.com/brronsuite/bronutil/txsort"
...
type PaymentDescriptor struct {
	...
	// Amount is the HTLC amount in milli-broneess.
...
func locateOutputIndex(p *PaymentDescriptor, tx *wire.MsgTx, ourCommit bool,
	...
	for i, txOut := range tx.TxOut {
		...
		if bytes.Equal(txOut.PkScript, pkScript) &&
			txOut.Value == int64(p.Amount.ToBroneess()) &&
			cltv == p.Timeout {
...
func (c *commitment) populateHtlcIndexes(chanType channeldb.ChannelType,
	...
	populateIndex := func(htlc *PaymentDescriptor, incoming bool) error {
		isDust := HtlcIsDust(
			...
			htlc.Amount.ToBroneess(), c.dustLimit,
		)
...
func (lc *LightningChannel) diskHtlcToPayDesc(feeRate chainfee.SatPerKWeight,
	...
	isDustLocal := HtlcIsDust(
		...
		htlc.Amt.ToBroneess(), lc.channelState.LocalChanCfg.DustLimit,
	)
	...
	isDustRemote := HtlcIsDust(
		...
		htlc.Amt.ToBroneess(), lc.channelState.RemoteChanCfg.DustLimit,
	)
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	case *lnwire.UpdateAddHTLC:
		...
		isDustRemote := HtlcIsDust(
			...
			wireMsg.Amount.ToBroneess(), remoteDustLimit,
		)
		if !isDustRemote {
			theirP2WSH, theirWitnessScript, err := genHtlcScript(
	...
	case *lnwire.UpdateFee:
		pd = &PaymentDescriptor{
			LogIndex: logUpdate.LogIndex,
			Amount: lnwire.NewMSatFromBroneess(
...
func NewBreachRetribution(chanState *channeldb.OpenChannel, stateNum uint64,
	...
	// Compute the balances in broneess.
	ourAmt := revokedSnapshot.LocalBalance.ToBroneess()
	theirAmt := revokedSnapshot.RemoteBalance.ToBroneess()
	...
	for _, htlc := range revokedSnapshot.Htlcs {
		...
		if HtlcIsDust(
			...
			htlc.Amt.ToBroneess(), chanState.RemoteChanCfg.DustLimit,
		) {
			continue
		}
		...
		htlcRetributions = append(htlcRetributions, HtlcRetribution{
			SignDesc: input.SignDescriptor{
				...
				Output: &wire.TxOut{
					...
					Value:    int64(htlc.Amt.ToBroneess()),
				},
				HashType: txscript.SigHashAll,
			},
			OutPoint: wire.OutPoint{
...
func (lc *LightningChannel) evaluateHTLCView(view *htlcView, ourBalance,
	theirBalance *lnwire.MilliBronees, nextHeight uint64,
	...
	for _, entry := range view.ourUpdates {
		...
		// number of broneess we've received within the channel.
	...
	for _, entry := range view.theirUpdates {
		...
		// tracking the total number of broneess we've sent within the
		// channel.
		if mutateState && entry.EntryType == Settle && !remoteChain &&
			entry.removeCommitHeightLocal == 0 {
...
func processAddEntry(htlc *PaymentDescriptor, ourBalance, theirBalance *lnwire.MilliBronees,
...
func processFeeUpdate(feeUpdate *PaymentDescriptor, nextHeight uint64,
	...
	view.feePerKw = chainfee.SatPerKWeight(feeUpdate.Amount.ToBroneess())
...
func genRemoteHtlcSigJobs(keyRing *CommitmentKeyRing,
	...
	for _, htlc := range remoteCommitView.incomingHTLCs {
		...
		htlcFee := HtlcTimeoutFee(chanType, feePerKw)
		outputAmt := htlc.Amount.ToBroneess() - htlcFee
	...
	for e := lc.localUpdateLog.Front(); e != nil; e = e.Next() {
		...
		case FeeUpdate:
			...
			logUpdate.UpdateMsg = &lnwire.UpdateFee{
				ChanID:   chanID,
				FeePerKw: uint32(pd.Amount.ToBroneess()),
			}
		}
...
func (lc *LightningChannel) validateCommitmentSanity(theirLogCounter,
	...
	validateUpdates := func(updates []*PaymentDescriptor,
		...
		var amtInFlight lnwire.MilliBronees
...
func (lc *LightningChannel) computeView(view *htlcView, remoteChain bool,
	updateState bool) (lnwire.MilliBronees, lnwire.MilliBronees, int64,
	*htlcView, error) {
	...
	for _, htlc := range filteredHTLCView.ourUpdates {
		if HtlcIsDust(
			...
			feePerKw, htlc.Amount.ToBroneess(), dustLimit,
		) {
			continue
		}
...
// format. See https://github.com/brronsuite/lightning-rfc/issues/728
//
...
func (lc *LightningChannel) GetDustSum(remote bool) lnwire.MilliBronees {
	lc.RLock()
	defer lc.RUnlock()

	var dustSum lnwire.MilliBronees
	...
	for e := lc.localUpdateLog.Front(); e != nil; e = e.Next() {
		...
		amt := pd.Amount.ToBroneess()

		// If the bronees amount is under the dust limit, add the msat
...
func (lc *LightningChannel) MayAddOutgoingHtlc(amt lnwire.MilliBronees) error {
	lc.Lock()
	defer lc.Unlock()

	var mockHtlcAmt lnwire.MilliBronees
	switch {
...
func newOutgoingHtlcResolution(signer input.Signer,
	...
	htlcFee := HtlcTimeoutFee(chanType, feePerKw)
	secondLevelOutputAmt := htlc.Amt.ToBroneess() - htlcFee
...
func (lc *LightningChannel) AvailableBalance() lnwire.MilliBronees {
	lc.RLock()
	defer lc.RUnlock()
...
func (lc *LightningChannel) availableBalance() (lnwire.MilliBronees, int64) {
...
func (lc *LightningChannel) availableCommitmentBalance(view *htlcView,
	remoteChain bool) (lnwire.MilliBronees, int64) {
	...
	ourReserve := lnwire.NewMSatFromBroneess(
		lc.channelState.LocalChanCfg.ChanReserve,
	)
	...
	feePerKw := filteredView.feePerKw
	htlcCommitFee := lnwire.NewMSatFromBroneess(
	...
	theirReserve := lnwire.NewMSatFromBroneess(
		lc.channelState.RemoteChanCfg.ChanReserve,
	)
	...
	dustlimit := lnwire.NewMSatFromBroneess(
...
func (lc *LightningChannel) validateFeeRate(feePerKw chainfee.SatPerKWeight) error {
	...
	oldFee := lnwire.NewMSatFromBroneess(
		lc.localCommitChain.tip().feePerKw.FeeForWeight(txWeight),
	)

	// Our base balance is the total amount of broneess we can commit
	...
	// this new fee update.
	newFee := lnwire.NewMSatFromBroneess(
...
func (lc *LightningChannel) UpdateFee(feePerKw chainfee.SatPerKWeight) error {
	...
	pd := &PaymentDescriptor{
		LogIndex:  lc.localUpdateLog.logIndex,
		Amount:    lnwire.NewMSatFromBroneess(bronutil.Amount(feePerKw)),
		EntryType: FeeUpdate,
	}
...
func (lc *LightningChannel) MaxFeeRate(maxAllocation float64) chainfee.SatPerKWeight {
	...
	baseBalance := availableBalance.ToBroneess() + oldFee
...
func (lc *LightningChannel) FwdMinHtlc() lnwire.MilliBronees {
