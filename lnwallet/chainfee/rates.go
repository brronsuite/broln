...
type SatPerKVByte bronutil.Amount

// FeeForVSize calculates the fee resulting from this fee rate and the given
// vsize in vbytes.
func (s SatPerKVByte) FeeForVSize(vbytes int64) bronutil.Amount {
	return bronutil.Amount(s) * bronutil.Amount(vbytes) / 1000
}
...
type SatPerKWeight bronutil.Amount

// FeeForWeight calculates the fee resulting from this fee rate and the given
// weight in weight units (wu).
func (s SatPerKWeight) FeeForWeight(wu int64) bronutil.Amount {
	// The resulting fee is rounded down, as specified in BOLT#03.
	return bronutil.Amount(s) * bronutil.Amount(wu) / 1000
}
